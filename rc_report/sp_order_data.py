...
def sp_order_data(count_file,db_method):

    file_path = get_latest_file_multiple_folder([os.path.join(os.getenv("BASE_RAW_FILE_PATH", ""), folder) for folder in shopee_order_path],n=count_file)
    ...
    write_table_by_unique_id(df,
                            target_table = 'rc_report.sp_order_data',
